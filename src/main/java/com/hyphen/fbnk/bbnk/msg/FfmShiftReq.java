...
public class FfmShiftReq {
    private static final Log log = LogFactory.getLog(FfmShiftReq.class);
    private final String encodeTp = "euc-kr";

    private String hRecTp       = "H";      //1
    private String hFileTp      = "FB0211"; //6
    ...
    private String getdFiller2  = "";       //254

    public FfmShiftReq() {}
    public FfmShiftReq(byte[] dataRec, byte[] headRec) {
        ...
        dFiller1     = new String(dataRec, 1+10+1+14, 1);
        socId        = new String(dataRec, 1+10+1+14+1, 13);
        befFacCd     = new String(dataRec, 1+10+1+14+1+13, 20);
        befCompCd    = new String(dataRec, 1+10+1+14+1+13+20, 10);
        befNapbuNo   = new String(dataRec, 1+10+1+14+1+13+20+10, 30);
        befBankCd    = new String(dataRec, 1+10+1+14+1+13+20+10+30, 3);
        befAcctNo    = new String(dataRec, 1+10+1+14+1+13+20+10+30+3, 20);
        aftBankCd    = new String(dataRec, 1+10+1+14+1+13+20+10+30+3+20, 3);
        aftAcctNo    = new String(dataRec, 1+10+1+14+1+13+20+10+30+3+20+3, 20);
        getdFiller2  = new String(dataRec, 1+10+1+14+1+13+20+10+30+3+20+3+20, 254);
    }

    public List<DtoShift> makeDtoList(String srcFilePath){
        FileInputStream fin = null;
        FfmShiftReq ffmShiftReq;
        ...
        try {
            ...
            while(true){
                ...
                if(new String(dataBuf, 0, 1).equals("H")){
                    ...
                    ffmShiftReq = new FfmShiftReq(dataBuf, headBuf);
                    shiftList.add(new DtoShift(ffmShiftReq.gethRegDt().trim(), ffmShiftReq.getVanId().trim(), ffmShiftReq.getShiftGb().trim(), ffmShiftReq.getShiftTm().trim(),
                            "", ffmShiftReq.getSocId().trim(), "", ffmShiftReq.getFacTp().trim(), ffmShiftReq.getBefFacCd().trim(), ffmShiftReq.getBefNapbuNo().trim(),
                            ffmShiftReq.getBefBankCd().trim(), ffmShiftReq.getBefAcctNo().trim(), "", "", ffmShiftReq.getAftBankCd().trim(),
                            ffmShiftReq.getAftAcctNo().trim(), "", "", "", "", ""));
                }
            }//while end.
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String getGetdFiller2() {
        return getdFiller2;
    }

    public void setGetdFiller2(String getdFiller2) {
        this.getdFiller2 = getdFiller2;
    }
}
