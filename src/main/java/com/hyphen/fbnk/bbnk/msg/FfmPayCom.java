...
public class FfmPayCom {
    private static final Log log = LogFactory.getLog(FfmPayCom.class);
    private final String encodeTp = "euc-kr";

    private String hRecTp   = "S";  //1
    private String jobTp    = "C2"; //2
    ...
    public FfmPayCom() {}
    public FfmPayCom(String facCd, String hBankCd, String acctSDt, String hAcctNo, String pwd, String repFacCd, String copNo, String amtTp, String roundNo, int dSeq, String dBankCd, String dAcctNo, long reqAmt, String itemCd, String bankBook, String mBankCd, String tmpInfo) {
        this.facCd = facCd;
        try{
    ...
    public FfmPayCom(byte[] dataRec, byte[] headRec) {
    ...
    public boolean makeFile(List<DtoPay> dtoPayList, String desFilePath){
        ...
        try {
            ...
            for (DtoPay dtoPay : dtoPayList) {
                dataSeq++;
                payCom = new FfmPayCom(dtoPay.getFacCd(), dtoPay.gethBankCd(), dtoPay.getAcctDt(), dtoPay.gethAcctNo(), dtoPay.getPwd(),
                        dtoPay.getRepFacCd(), dtoPay.getCopNo(), dtoPay.getAmtTp(), dtoPay.getRoundNo(), dataSeq, dtoPay.getdBankCd(), dtoPay.getdAcctNo(),
                        dtoPay.getReqAmt(), dtoPay.getItemCd(), dtoPay.getBankBook(), dtoPay.getmBankCd(), dtoPay.getTmpInfo());
                ttReqAmt += dtoPay.getReqAmt();
                //make head
                if(dataSeq==1) fout.write(payCom.getHead());
                //make data
                fout.write(payCom.getData());
                //make end
                if(dataSeq== dtoPayList.size()) fout.write(payCom.getEnd(dataSeq, ttReqAmt));
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ...
            while (true){
                ...
                if(new String(dataBuf, 0, 1).equals("S")){
                    ...
                    payCom = new FfmPayCom(dataBuf, headBuf);
                    payList.add(new DtoPay(payCom.getFacCd().trim(), payCom.gethBankCd().trim(), payCom.getAcctSDt().trim(), payCom.gethAcctNo().trim(),
                            payCom.getPwd().trim(), payCom.getRepFacCd().trim(), payCom.getCopNo().trim(), payCom.getAmtTp().trim(), payCom.getRoundNo().trim(),
                            payCom.getdBankCd().trim(), payCom.getdAcctNo().trim(), Long.parseLong(payCom.getReqAmt()), payCom.getResult().trim(),
                            payCom.getErrCd().trim(), Long.parseLong(payCom.getFailAmt()), payCom.getItemCd().trim(),
                            payCom.getBankBook().trim(), payCom.getmBankCd().trim(), payCom.getTmpInfo().trim()));
                }
            }//while end.
        } catch (Exception e) {
            e.printStackTrace();
